  ...
  <div>
    <section :class="{ 'page-skeleton-loading': initialLoading }">
      <section class="hero-card">
        ...
        <div class="hero-actions"><el-button @click="$emit('navigate', 'holdings')">查看持仓收益</el-button></div>
      </section>
      ...
      <el-row :gutter="18" class="home-content-row">
        <el-col :xs="24" :lg="14">
          <el-card shadow="never" class="panel-card home-sector-card">
            <div slot="header" class="panel-header"><span>板块排行</span><el-button type="text" @click="$emit('navigate', 'market')">完整复盘</el-button></div>
            <div v-if="initialLoading" class="home-sector-skeleton"><i v-for="row in 5" :key="row" /></div>
        ...
        <el-col :xs="24" :lg="10">
          <el-card shadow="never" class="panel-card news-source-card home-news-card">
            <div slot="header" class="panel-header"><span>最新消息 · 东方财富</span><el-button type="text" @click="$emit('navigate', 'news')">更多</el-button></div>
