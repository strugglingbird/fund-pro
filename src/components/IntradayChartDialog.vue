...
export default {
  ...
  methods: {
    ...
    async loadFundHistory() {
      if (!this.activeCode || this.fundHistoryRange.length !== 2) return
    ...
    renderIntradayChart() {
      ...
      this.intradayInstance = this.intradayInstance || echarts.init(container)
    ...
    renderFundPerformance() {
      ...
      this.performanceInstance = this.performanceInstance || echarts.init(container)
