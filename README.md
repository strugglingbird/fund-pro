...
面向个人投资跟踪的全栈工作台，集中展示持仓估值、市场指数、板块复盘、资金轮动和财经快讯。项目同时支持浏览器访问与 Android APK 打包。
...
- **板块复盘**：同花顺行业涨跌幅前 10 名、板块资金轮动回放；资金快照每 30 秒采样，并仅保留近 1 周数据。
...
| `GET /api/sector-fund-flow` | 板块资金轮动序列 |
| `GET /api/news` | 财经快讯 |
